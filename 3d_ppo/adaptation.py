...
ppo = PPO(3, 2, method = 'penalty')
ppo.load_model(4999, 1)
...
Individual = IndividualModel(state_size=3, action_size=1, seed=0, fc1_units=25).to(device)
	...
	for ep in range(EP_NUM):
		...
		for t in range(env.max_iteration):
			...
			if mode == 'switch':
				...
				control_action = (0.634*state[0] - 0.296*state[1] - 0.153*state[2] 
				+ 0.053*state[0]**2 - 1.215*state[0]**3).cpu().data.numpy()
				if ep == 0:
...
if __name__ == '__main__':
	...
	# _, sw_fuel, _, _ = test('./adapter_hard/ddqn_1900_5.pth', state_list=state_list, renew=False, mode='switch')
	...
	# np.save('../tmp/3d_direct.npy', indi_action)
	# np.save('../tmp/3d_robust.npy', robust_action)
