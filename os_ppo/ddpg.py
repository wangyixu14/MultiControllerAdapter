...
def test(modelname, renew, state_list=[], EP_NUM=500, random_initial_test=True):
	...
	for ep in range(EP_NUM):
		...
		if renew:
			while True:
				state = env.reset()
				if where_inv_valuebased_direct(state):
					break
			# state = env.reset()
	...
	return state_list, fuel_list, trajectory


def where_inv_valuebased_star(state):
	x = state[0] / 3
	y = state[1] / 3
...
if __name__ == '__main__':
	# print(where_inv_valuebased([1.9,  0.72447223]))
	# assert False
	...
	# state_list, fuel_list, trajectory = test('./robust_distill_0915.pth', renew=False, state_list=[[1.8,  -1.5]], EP_NUM=1)
	# print(np.mean(fuel_list), len(fuel_list))
	# np.save('./plot/line4.npy', np.array(trajectory))

	state_list, fuel_list, _ = test('./direct_distill.pth', renew=True, state_list=[], EP_NUM=1000)
	print(len(fuel_list))
